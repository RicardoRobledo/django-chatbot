...
use the "../../static/json/config.json" file to make your settings. /*/
let data_index = "";
let employee_name = "";
let employee_expert = "";
let employee_image = "";
...
let base_url = 'https://django-chatbot-amqo.onrender.com/api/';
let data_procedures = [];
let procedure = '';
...
loadData("../../static/json/config.json", ["../../static/json/employees.json", "../../static/json/lang.json", "../../static/json/badwords.json"]);
	...
	return Promise.all([fetch(url).then(res => res.json()), ...urls.map(url => fetch(url).then(res => res.json()))])
		.then(([out, OutC, OutL, OutB]) => {
			...
			for (var i = 0; i < OutC.length; i++) {
				...
				max_num_chats_api = OutC[i]['max_num_chats_api'];
				employee_name = OutC[i]['name'];
				employee_expert = OutC[i]['description'];
				employee_image = OutC[i]['image'];
				welcome_message = OutC[i]['welcome_message'];
				google_voice = OutC[i]['google_voice'];
				google_voice_lang_code = OutC[i]['google_voice_lang_code'];
			}
      ...
      ${avatar_in_chat}
        ...
        <div class="user-name"><h5>${employee_name}</h5></div>
          <div class="message-text">
            <div class="chat-response">${welcome_message}</div>
          </div>
...
`);
	...
	$(".ai-contacts-top input[type='text']").on("input", function(event) {
		...
		for (const json of data_procedures) {
			if (json.text.includes(text)) {  // Usando `includes()` para búsqueda simple
				filtered_data_procedure.push(json);
			}
		}

		list_data(filtered_data_procedure);
	...
	$(".ai-contacts-top input[type='text']").prop("disabled", true);
...
$(document).ready(function() {
	// Realiza la petición AJAX al cargar la página
	$.ajax({
	  url: `${base_url}bcs/`,
	  type: 'GET', // o 'POST' u otro método HTTP según tus necesidades
	  dataType: 'json', // el tipo de datos que esperas recibir del servidor
	  success: function(data) {

		data_procedures = data.map((json, index)=>{
		    json['index'] = index;
			return json;
		});

		list_data(data_procedures);
		$(".ai-contacts-top input[type='text']").prop("disabled", false);
	  },
...
function list_data(data){
	...
	for (var i = 0; i < data.length; i++) {
		...
		$(".ai-contacts-scroll").append(`
			<div onclick="select_procedure('${index}', '${element}')" class="text-card-container" id="${index}">
				${element}
			</div>
			<br>
		`);
...
$(window).on('beforeunload', function() {
	...
	var requestOptions = {
		...
		body: JSON.stringify({'thread_id':localStorage.getItem('thread_id')}),
	};

	navigator.sendBeacon(`${base_url}chatbot/cleaner_manager/`, requestOptions);
...
async function getResponse(prompt) {
	...
	prompt += ".↵↵ Escribe en tono gentil.";
	prompt += "↵↵ Ten un estilo de escritura formal.";
	prompt += "↵↵ Response en Español.";
	...
	params.append('index', index_procedure);
	...
	try {
		...
		source = new SSE(`${base_url}chatbot/`, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, payload: params, method: 'POST' });
        ...
        <div class="conversation-thread thread-ai">
          ...
          <div class="message-container">
            <div class="message-info">
              ...
              <div class="date-chat"><img src="../../static/img/icon-clock.svg"> ${currentDate()}</div>
            </div>
          </div>
        </div>
			`);
		...
		.then(data => {
			if (data.status == 1) {
				...
				for (let i = 0; i < images.length; i++) {
					$(".image_ia_" + imageID).append(`<div class="image-ia"><img onerror="this.src='../../static/img/no-image.svg'" src="${images[i].url}"></div>`)
				}
...
function displayChat(element, index = "") {
	...
	/*$(".ai-chat-top-image img").attr("src", "https://www.google.com/url?sa=i&url=https%3A%2F%2Fes.picmix.com%2Fstamp%2FRockman-X-Dive-1980288&psig=AOvVaw37Wr2Zg5u0mwKDExhWefW-&ust=1703274867974000&source=images&cd=vfe&opi=89978449&ved=0CBEQjRxqFwoTCMjJjsKnoYMDFQAAAAAdAAAAABAD")
	$(".ai-chat-top-name").html(`<h4>${employee_name} <span class="online-bullet"></span></h4>`)
	$(".ai-chat-top-job").html(employee_expert)*/
		...
		for (var i = 0; i < array_employees[data_index]['last_chat'].length; i++) {
			...
			if (currentChat.name === "User") {
				if (currentChat.isImg === true) {
					...
					const imgHtml = Array.isArray(currentChat.imgURL) ? currentChat.imgURL.map(url => `<div class="image-ia"><img onerror="this.src='../../static/img/no-image.svg'" src="${url}"></div>`).join('') : '';
					const chatHtml = `
				        <div class="conversation-thread thread-ai">
				          <div class="message-container">
				            <div class="message-info">
				              ...
				              <div class='date-chat'><img src='../../static/img/icon-clock.svg'> ${currentChat.date || ''}</div>
				          </div>
				        </div>
		          `;
				...
				avatar_in_chat = display_avatar_in_chat ? `<div class="user-image"><img onerror="this.src='../../static/img/no-image.svg'" src="${employee_image}" alt="${employee_name}" title="${employee_name}"></div>` : '';
				audio_in_chat = display_audio_button_answers ? `<div class='chat-audio'><img data-play="false" src='../../static/img/btn_tts_play.svg'></div>` : '';
...
$(document).on("click", ".chat-audio", function () {
	...
	$img.attr({
		"src": "../../static/img/btn_tts_" + (play ? "play" : "stop") + ".svg",
...
function doSpeechSynthesis(longText, chatResponse) {
	...
	utterances[utterances.length - 1].addEventListener("end", () => {
		$(".chat-audio img").attr("src", "../../static/img/btn_tts_play.svg");
...
if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
	...
	recognition.addEventListener('start', () => {
		...
		$("#microphone-button").attr("src", "../../static/img/mic-stop.svg")
	});
	...
	recognition.addEventListener('end', () => {
		...
		$("#microphone-button").attr("src", "../../static/img/mic-start.svg")
